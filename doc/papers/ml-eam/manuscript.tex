...
key constants, including $N^{vap}$ and $N^{\mathrm{max}}_{\mathrm{ij}}$. 
$N^{\mathrm{max}}_{\mathrm{ijk}}$ is not needed as neither EAM nor ADP contains 
triple-atoms interaction. Then we can build the global symbol list (GSL). Hence,
structures of any stoichiometries can be transformed to their corresponding 
unversal GSL forms for batching. The descriptors (\ref{sec:transformation}) are 
