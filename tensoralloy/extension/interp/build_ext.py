...
def get_interp_extension() -> setuptools.Extension:
    """
    Construct an external op.
    """
    path = join(dirname(__file__), "src")
    cpp_files = glob.glob(join(path, "*.cc"))
    cuda_files = glob.glob(join(path, "*.cu"))

    flags = []
    if sys.platform == "darwin":
        flags += ["-mmacosx-version-min=10.9"]
    ext_name = f"tensoralloy.extension.interp.cubic_op"
...
class BuildExtension(build_ext):
    """
    Build the tensorflow extension.
    """
